...
- **Unpinned jupyterlab breaks venv** — `uv pip install jupyterlab` without a version constraint could pull a release incompatible with existing Money Maker venv packages. Pinned to `>=4.0,<5` range to bound resolution.
- **`uv` not in PATH during Docker build** — switched from bare `uv` to explicit `/opt/moneymaker/.venv/bin/uv` so the install works regardless of base image PATH configuration.
...
- **Chromium browser tools** — installs Chromium and display/font libs so Money Maker browser-use tools work out of the box.
- **Plugin persistence** — Money Maker plugin directory symlinked into the persistent volume; plugins survive container restarts.
- **Secret redaction** — enabled by default in Money Maker config (`security.redact_secrets: true`).
...
- **moneymaker not found in terminal** — symlink `moneymaker` CLI into `$MONEY_MAKER_HOME/.local/bin`; add `/etc/profile.d/moneymaker-venv.sh` so PATH includes venv bin in all shell types.
...
- Space emoji updated to 🪽 (Money Maker winged sandals) across README and dashboard.
...
- Initial Money Maker 🤑 Docker Space wrapper for Money Maker Money Maker 🤑.
...
- Added private HF Dataset backup and restore for Money Maker state.
