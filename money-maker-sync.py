...
MONEY_MAKER_HOME = Path(os.environ.get("MONEY_MAKER_HOME", "/opt/data"))
STATUS_FILE = Path("/tmp/huggingmes-sync-status.json")
STATE_FILE = MONEY_MAKER_HOME / ".huggingmes-sync-state.json"
...
def write_status(status: str, message: str, fingerprint: str | None = None, marker: tuple[int, int, int] | None = None) -> None:
    ...
    # Update persistent state file in MONEY_MAKER_HOME
...
def restore() -> bool:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            MONEY_MAKER_HOME.mkdir(parents=True, exist_ok=True)
            for child in tmp_path.iterdir():
                if should_exclude(child.name, child):
                    continue
                target = MONEY_MAKER_HOME / child.name
                if target.is_dir():
...
def sync_once(last_fingerprint: str | None = None, last_marker: tuple[int, int, int] | None = None):
    ...
    current_marker = metadata_marker(MONEY_MAKER_HOME)
    ...
    current_fingerprint = fingerprint_dir(MONEY_MAKER_HOME)
    ...
    snapshot_dir = create_snapshot_dir(MONEY_MAKER_HOME)
    try:
        upload_folder(
...
def loop() -> int:
    ...
    last_fingerprint = fingerprint_dir(MONEY_MAKER_HOME)
    last_marker = metadata_marker(MONEY_MAKER_HOME)
