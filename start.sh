...
mkdir -p "${MONEY_MAKER_HOME}/skills"
...
if [ -f "${MONEY_MAKER_HOME}/requirements.txt" ]; then
    echo "Installing persistent requirements..."
    /opt/hermes/.venv/bin/python -m pip install -r "${MONEY_MAKER_HOME}/requirements.txt" --quiet
fi
